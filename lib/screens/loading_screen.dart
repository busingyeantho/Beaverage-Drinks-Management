...
class _LoadingScreenState extends State<LoadingScreen> {
  ...
  List<LoadingReturn> _loadingData = [];
  ...
  Future<void> _setupSheetHeaders() async {
    try {
      ...
      await _googleSheetsService.appendRow('LOADINGS!A:Z', headers);
    } catch (e) {
  ...
  Future<void> _submitForm() async {
    if (_formKey.currentState!.validate()) {
      ...
      try {
        final loadingData = LoadingReturn(
          date: _selectedDate,
        ...
        final values = loadingData.toSheetsRow();

        // Append to the LOADINGS sheet
        await _googleSheetsService.appendRow('LOADINGS!A:Z', values);
  ...
  Widget build(BuildContext context) {
    return DefaultTabController(
      ...
      child: Scaffold(
        appBar: AppBar(
          title: const Text('Morning Loadings'),
          ...
          bottom: const TabBar(
            tabs: [
              Tab(icon: Icon(Icons.list), text: 'View Loadings'),
              Tab(icon: Icon(Icons.add), text: 'Add Loading'),
            ],
          ),
        ),
        body: TabBarView(
    ...
    return Form(
      ...
      child: Column(
        ...
        children: [
          ...
          const Text(
            'You can select any date (past or future) for when the loading actually occurred',
